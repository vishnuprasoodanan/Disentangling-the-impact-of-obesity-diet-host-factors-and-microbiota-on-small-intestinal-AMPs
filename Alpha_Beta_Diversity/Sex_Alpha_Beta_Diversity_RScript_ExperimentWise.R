...
#--------------------------------------------------------
#Colors <- c("darkolivegreen4", "red") #colors used for different sexs
Colors <- c("lightblue", "gray") #colors used for gender

#Colors1 <- c("darkolivegreen","red4") #colors used for different sexs
...
for (level in factor_levels) {
  ...
  for (level2 in factor_levels2) {
    ...
    file_name5 <- paste(level, level2, "_alpha_diversity_estimates_sex.pdf", sep = "_")
    file_name6 <- paste(level, level2, "_alpha_diversity_estimates_sex.jpg", sep = "_")
    ...
    p <- plot_richness(ps, "sex", measures = c("Observed", "Chao1", "Shannon"), color = NULL, shape = NULL)
    p <- p + geom_boxplot(data = p$data, aes(x= sex, y = value, fill = sex)) + 
      labs(x="",y="Alpha Diversity Measure") + 
      theme_bw() +
      scale_color_manual(values = Colors1)+
      scale_fill_manual(values = Colors)+
      geom_point(aes(colour = factor(sex)), position=position_jitterdodge(jitter.width = 1), size = 5) +
    ...
    plot(Uni_pcoa$vectors[,1:2], bg= c("lightblue", "gray")[as.factor(Bushman2@sam_data$sex)], pch=21, cex=2, xlab=paste0("PCoA", pc[1], " (", mds.var.per[1], "%)"), ylab=paste0("PCoA", pc[2], " (", mds.var.per[2], "%)"))
    text(Uni_pcoa$vectors[,1:2], labels=rownames(Bushman2@sam_data), cex=0.3, font=1, pos=1)
    ordiellipse(Uni_pcoa$vectors[,1:2], Bushman2@sam_data$sex, kind="sd", lwd=1, lty=3, draw = "polygon", alpha = 50, col = c("lightblue", "gray"))
    ordispider(Uni_pcoa$vectors[,1:2], Bushman2@sam_data$sex, lty=3, spider ="centroid", lwd=1, col="black")
    legend("bottomleft", legend = c("Female", "Male"), col = c("lightblue", "gray"),lty = c(1,1,1,1), cex=0.7, title = "", border = "white", fill = NULL, bg="white", bty = "n")
    abline(h=0, v=0, col = "gray60")
    dev.off ()
    adonis2(UniFrac_distances ~ Bushman2@sam_data$sex)
    ...
    abline(h=0, v=0, col = "gray60")
    dev.off ()
    adonis2(UWUniFrac_distances ~ Bushman2@sam_data$sex)
    ...
    ordiellipse(Uni_pcoa$vectors[,1:2], Bushman2@sam_data$sex, kind="sd", lwd=1, lty=3, draw = "polygon", alpha = 50, col = c("lightblue", "gray"))
    ordispider(Uni_pcoa$vectors[,1:2], Bushman2@sam_data$sex, lty=3, spider ="centroid", lwd=1, col="black")
    legend("topleft", legend = c("Female", "Male"), col = c("lightblue", "gray"),lty = c(1,1,1,1), cex=0.7, title = "", border = "white", fill = NULL, bg="white", bty = "n")
    abline(h=0, v=0, col = "gray60")
    dev.off ()
    adonis2(BC_distances ~ Bushman2@sam_data$sex)
    rm(ps)
  }
}
